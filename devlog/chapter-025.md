...
### 후속 조정 2 (민호 피드백, 같은 날)
- **헤더를 Court Lime 으로** — reversed-lime lockup 그대로, 라임 위 텍스트는
  전부 잉크 계열. 워드마크에 on="lime" 모드 추가 (라임 위 lab 은 잉크).
- **히어로 배경을 광고 포스터 수준으로** — opacity 75, 헤드 클로즈업 크롭,
  그라데이션 최소화. 브랜드 공식 광고 사진은 로컬에 없고 실제 광고 이미지
  사용은 저작권 문제라, 보유한 2026년형 촬영 렌더를 광고처럼 키우는 방식.
- **퀵링크 배너 세련화** — 원색 3종 → 차분한 에디토리얼 3종:
  비교 = 네이비 항공(네트 대각선), DNA = 클레이 위 선수 그림자,
  스트링 = 세이지 프레임 매크로 (Unsplash License, 파일명에 사진 ID).
