...
- 라켓의 파워, 컨트롤, 스핀, 편안함, 안정성을 우리 언어로 점수화한 해석 체계가 핵심 제품 자산이다.
- 사용자 노출 라켓 점수는 파워, 컨트롤, 스핀, 편안함, 안정성의 5축으로 제한한다.
- 공개 UI는 각 축을 정수 `N/5`, 다섯 축 합계를 정수 `N/15`로 표시하고 소수점을 사용하지 않는다.
- 내부 raw v3 `0~100`은 공개 정수로 덮어쓰지 않고 정렬과 추천에 사용한다.
