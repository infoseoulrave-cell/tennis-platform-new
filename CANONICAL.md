...
- 데이터 일치: 홈과 상세가 동일한 canonical 점수 원본을 사용함
...
- raw v3 5축 점수는 내부 `0..100`으로 보존하고 정렬·추천에는 이 raw 값을 사용한다.
- 공개 5축은 각각 정수 `0..5`, 총점은 정수 `10..15`로 표시하며 소수점을 사용하지 않는다.
- 목표 총점은 `round(10 + rawSum / 100)`이고, bounded projection과 deterministic largest-remainder로 공개 5축 정수의 합을 목표 총점과 정확히 일치시킨다.
...
- 적용 후 프로덕션 목록·상세·비교·가이드, 축별 `N/5`와 총 `N/15` 정수 표시, 은퇴 중복 차단, DB 집계와 Omega 무변경 상태를 다시 검증한다.
- 후속 P2는 은퇴 중복 데이터의 DB·관리자 수집 처리와 미래 후보의 evidence role 강제다.

## 9. 2026-07-24 LOCAL 변경 상태

이 절은 로컬에서 검증한 score/player 변경을 기록하며 운영 반영 완료를 뜻하지 않는다.

- 작업 상태는 `로컬 미커밋`이며 새 프리뷰 URL과 배포 ID는 아직 없다.
- 새 프리뷰 검증 예정이며 Git push, 프로덕션 배포, DB 적용은 수행하지 않았다.
- Omega URL `https://racketlab-omega.vercel.app/`와 배포 ID `dpl_GmX79sYrsM78DpRFmLLnoktrtYXv`는 계속 불변이다.

### 공개 점수 계약

- canonical 공개 축 순서는 파워, 컨트롤, 스핀, 편안함, 안정성이다.
- raw v3 5축은 각각 내부 `0..100`으로 유지하며 정렬과 추천은 반올림되지 않은 raw를 사용한다.
- raw 합을 `rawSum`이라 할 때 공개 목표 총점은 `round(10 + rawSum / 100)`으로 계산하고 결과는 항상 정수 `10..15`다.
- raw 축을 먼저 `raw / 20`으로 옮긴 뒤 `0..5` 경계 안에서 합이 목표 총점이 되도록 bounded projection을 수행한다.
- 각 투영값의 정수부를 배정한 뒤 largest-remainder로 남은 점수를 배분하며, 동률은 raw 내림차순과 파워·컨트롤·스핀·편안함·안정성 순서로 결정한다.
- 최종 공개 5축은 각각 정수 `0..5`이고 그 합은 목표 총점과 정확히 같아야 한다.
- UI는 축을 `N/5`, 총점을 `N/15`로 표시하며 소수점을 노출하지 않는다.

### 선수 콘텐츠 계약

- 공개 선수는 20명이며 각 선수의 기능성 카피는 플레이 스타일과 시판 라켓 라인을 연결하는 정확히 두 문장으로 유지한다.
- Wikimedia Commons 사진 URL은 안정적인 `250px` 썸네일로 정규화하고 출처·크레딧·라이선스를 보존한다.
- 장비 line은 최신 공식 선수 또는 브랜드 출처로 정밀화하고 프로 스톡과 시판 라인의 차이를 오해시키지 않는다.
